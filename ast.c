...
voba_value_t create_toplevel_env(voba_value_t module)
{
    ...
    TOPLEVEL_ENV(r)->a_ast_toplevel_vars = voba_make_array_0();
...
voba_value_t make_ast_top_var(voba_value_t value, voba_value_t module_id, enum ast_top_var_flag flag)
{
    ...
    AST(r)->u.top_var.module_id = module_id;
    AST(r)->u.top_var.flag = flag;
    /* if(voba_is_nil(module_id)){ */
    /*     fprintf(stderr,__FILE__ ":%d:[%s] nil\n", __LINE__, __FUNCTION__); */
    /* }else{ */
    /*     fprintf(stderr,__FILE__ ":%d:[%s] %s\n", __LINE__, __FUNCTION__,voba_str_to_cstr(voba_value_to_str(module_id))); */
    /* } */
    return r;
}
