...
static voba_value_t compile_top_expr_def_fun_next(voba_value_t self, voba_value_t args) 
{
    ...
    voba_value_t syn_s_name = voba_tuple_at(self,1);
    voba_value_t la_syn_arg_list = voba_tuple_at(self,2);
    voba_value_t la_syn_body = voba_tuple_at(self,3);
    voba_value_t env = voba_tuple_at(self,4);
    VOBA_ASSERT_N_ARG( args, 0); voba_value_t toplevel_env = voba_tuple_at( args, 0);
;
    // -------------------------------
    ...
    return make_ast_set_var(top_var, ast_exprs);
}
    ...
    if(xlen > 1){
        ...
        if(voba_is_a(syn_s_name,voba_cls_syn)&&
           voba_is_a(SYNTAX(syn_s_name)->v,voba_cls_symbol)){
            uint32_t offset = 1; // skip function name;
            voba_value_t la_syn_arg_list = voba_la_from_array1(x_form,offset);
            voba_value_t la_exprs = voba_la_from_array1(top_expr,2);
            voba_value_t top_var = create_topleve_var_for_def(syn_s_name,toplevel_env);
            voba_value_t env = TOPLEVEL_ENV(toplevel_env)->env;
            voba_value_t next = voba_make_closure_5(
                compile_top_expr_def_fun_next,
                top_var, syn_s_name, la_syn_arg_list, la_exprs, env);
